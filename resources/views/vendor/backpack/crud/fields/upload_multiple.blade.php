    	...
    	@foreach($values as $key => $file_path)
    		<div class="file-preview">
    			@if (isset($field['temporary']))
		            <a target="_blank" href="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->temporaryUrl($prefix.$file_path, Carbon\Carbon::now()->addMinutes($field['temporary']))):asset($file_path) }}">{{ $file_path }}</a>
		        @else
		            <a target="_blank" href="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->url($prefix.$file_path)):asset($prefix.$file_path) }}">{{ $file_path }}</a>
		        @endif
				...
				<img class="img-fluid col-sm-6 pb-2" src="{{ asset($prefix.$file_path) }}" height="250">
	    	</div>
    	@endforeach
    </div>
    @endif
        	...
        	function bpFieldInitUploadMultipleElement(element) {
		        ...
		        fileInput.change(function() {
					...
					if(selectedFiles.length === 1){
						inputLabel.html(selectedFiles.length + " {{ __('backpack.upload_multiple_files_selected_one') }}");
					}else{
						inputLabel.html(selectedFiles.length + " {{ __('backpack.upload_multiple_files_selected') }}");
					}
