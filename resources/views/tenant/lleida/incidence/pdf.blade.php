...
<body>
    ...
    <div class="container">
        ...
        <div class="row" style="padding-top:30px;">
            <div class="col-xs-11" style="font-size: 80%">
                @if($entry->type === 'owner_incidence' || $entry->type === 'specific_activities')
                    A la ciutat de Lleida, en data <b>{{$entry->date_incidence->format("d-m-Y")}}</b>,
                    en/na <b>{{$entry->user->name}}</b> en qualitat de vigilant de l'àrea de Mercats i personal de la Regidoria de Consum, Comerç, Mercats i Participació de Lleida
                    amb seu social a Av. Tortosa, 2, es persona a la/les parada/es núm. <b>{{$entry->stall->num}}</b>, de la
                ...
                @elseif($entry->type === 'general_incidence')
                    A la ciutat de Lleida, en data <b>{{$entry->date_incidence->format("d-m-Y")}}</b>, el vigilant <b>{{$entry->user->name}}</b> 
                    en qualitat de vigilant de l'àrea de Mercats i personal de la Regidoria de Consum, Comerç, Mercats i Participació de Lleida
                    amb seu social a Av. Tortosa, 2.
                @endif
            </div>
        </div>
        <div class="row" style="padding-top:30px;">
        ...
        @if($entry->images)
        @foreach(collect($entry->images)->chunk(2) as $chunks) 
        <div class="row" style="padding-top:30px;">
            @foreach($chunks as  $image)
            <div class="col-xs-6">
                <img class="photo" src="{{ storage_path('/app/'.$image) }}">
            </div>
            @endforeach
        </div>
        @endforeach
        ...
        <div class="row" style="padding-top:60px;">
            <div class="col-xs-11" style="font-size: 80%">
                En conformitat amb els fets descrits anteriorment, el vigilant <b>{{$entry->user->name}}</b> signa la present acta a <b>{{date("d-m-Y")}}</b>.
            </div>
        </div>
