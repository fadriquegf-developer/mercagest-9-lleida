...
class AbsenceRequestUpdate extends FormRequest
{
    /**
    ...
    {
        $abcence = Absence::findOrFail($this->input('id'));
        $result = Absence::where('id', '!=', $abcence->id)->where('stall_id', $abcence->stall_id)->byBusy($this->input('start'), $this->input('end'))->count();
